...
public class InventoryViewWrapper extends InventoryViewMock {
    ...
    @Nonnull
    public static InventoryViewWrapper wrap(@Nonnull HumanEntity player, @Nonnull String name, @Nonnull Inventory top, @Nonnull Inventory bottom, @Nonnull InventoryType type) {
        return new InventoryViewWrapper(player, name, top, bottom, type);
    }
    ...
    public org.bukkit.inventory.MenuType getMenuType() {
        return getType().getMenuType();
    }

    @Override
    public void open() {
        // No-op for the test wrapper; MockBukkit handles view lifecycle differently.
    }
