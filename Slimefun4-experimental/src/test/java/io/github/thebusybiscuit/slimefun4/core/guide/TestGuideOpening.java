...
class TestGuideOpening {
    ...
    void testOpenSearch_withColoredSearchTerm() throws InterruptedException {
        ...
        Assertions.assertEquals(InventoryType.CHEST, player.getOpenInventory().getTopInventory().getType(), "Failed on normal query");

        guide.openSearch(profile, coloredTerm, false);
        // Assert we can open with a coloured search term
        Assertions.assertEquals(InventoryType.CHEST, player.getOpenInventory().getTopInventory().getType(), "Failed on colored query");
    }
