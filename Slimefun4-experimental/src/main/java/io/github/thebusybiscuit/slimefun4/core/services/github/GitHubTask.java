...
class GitHubTask implements Runnable {
    ...
    private @Nullable String pullTexture(@Nonnull Contributor contributor, @Nonnull Map<String, String> skins) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        if (uuid.isPresent()) {
            try {
                Object profile = createPlayerProfile(uuid.get(), contributor.getMinecraftName());
                if (profile == null) {
                    return null;
                }

                @SuppressWarnings("unchecked")
                CompletableFuture<Object> future = (CompletableFuture<Object>) profile.getClass().getMethod("update").invoke(profile);
                Object updatedProfile = future.get(30, TimeUnit.SECONDS);
                Object textures = updatedProfile.getClass().getMethod("getTextures").invoke(updatedProfile);
                Object skin = textures.getClass().getMethod("getSkin").invoke(textures);

                if (skin != null) {
                    String skinUrl = skin.toString();
                    skins.put(contributor.getMinecraftName(), skinUrl);
                    return skinUrl;
                }
    ...
    private @Nullable Object createPlayerProfile(@Nonnull UUID uuid, @Nonnull String name) {
        try {
            return Bukkit.class.getMethod("createPlayerProfile", UUID.class, String.class).invoke(null, uuid, name);
        } catch (ReflectiveOperationException ignored) {
            try {
                return Bukkit.class.getMethod("createPlayerProfile", UUID.class).invoke(null, uuid);
