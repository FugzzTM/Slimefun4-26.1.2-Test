...
public class ChestMenu {
    ...
    private void setup() {
        ...
        for (int i = 0; i < this.items.size(); i++) {
            this.inv.setItem(i, ensureNativeStack(this.items.get(i)));
        }
    }
    ...
    public void replaceExistingItem(int slot, ItemStack item) {
        setup();
        this.inv.setItem(slot, ensureNativeStack(item));
    }

    /**
     * Converts any {@link ItemStack} subclass (e.g. SlimefunItemStack) into a
     * plain {@link ItemStack} that CraftBukkit's {@code CraftInventory.setItem()}
     * can safely pass to {@code CraftItemStack.asNMSCopy()}.
     * <p>
     * On Purpur 26.1.2+ (and potentially future Paper builds),
     * {@code CraftItemStack.asNMSCopy()} casts the input to
     * {@code CraftItemStack}, which fails for custom subclasses that
     * extend the abstract {@link ItemStack} instead of {@code CraftItemStack}.
     * Rebuilding the stack from its type, amount, and meta avoids the cast.
     */
    private static ItemStack ensureNativeStack(ItemStack item) {
        if (item == null || item.getType() == Material.AIR) {
            return item;
        }

        // If it's already a plain ItemStack (or CraftItemStack) this is cheap.
        // For custom subclasses like SlimefunItemStack this rebuilds as a
        // plain ItemStack that CraftBukkit can convert.
        ItemStack safe = new ItemStack(item.getType(), item.getAmount());
        if (item.hasItemMeta()) {
            safe.setItemMeta(item.getItemMeta());
        }
        return safe;
    }
