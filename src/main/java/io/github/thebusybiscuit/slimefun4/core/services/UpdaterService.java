...
public class UpdaterService {
    ...
    public UpdaterService(@Nonnull Slimefun plugin, @Nonnull String version, @Nonnull File file) {
        ...
        if (version.contains("UNOFFICIAL")) {
            ...
            try {
                autoUpdater = new BlobBuildUpdater(versionedPlugin(version), file, "Slimefun4", "Dev");
            } catch (Exception x) {
            ...
            try {
                autoUpdater = new BlobBuildUpdater(versionedPlugin(version), file, "Slimefun4", "RC");
            } catch (Exception x) {
                plugin.getLogger().log(Level.SEVERE, "Failed to create AutoUpdater", x);
            }
    ...
    private Plugin versionedPlugin(String version) {
        PluginDescriptionFile description = plugin.getDescription();
        PluginDescriptionFile versionedDescription = new PluginDescriptionFile(description.getName(), version, description.getMain());

        InvocationHandler handler = (proxy, method, args) -> {
            if ("getDescription".equals(method.getName())) {
                return versionedDescription;
            }

            if ("equals".equals(method.getName())) {
                return proxy == args[0] || plugin.equals(args[0]);
            }
            ...
            Method target = plugin.getClass().getMethod(method.getName(), method.getParameterTypes());
            return target.invoke(plugin, args);
        };

        return (Plugin) Proxy.newProxyInstance(plugin.getClass().getClassLoader(), new Class<?>[] { Plugin.class }, handler);
    }
