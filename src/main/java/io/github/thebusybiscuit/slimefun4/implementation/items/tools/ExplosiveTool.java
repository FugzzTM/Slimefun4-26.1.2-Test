...
public class ExplosiveTool extends SimpleSlimefunItem<ToolUseHandler> implements NotPlaceable, DamageableItem {
    ...
    private static Constructor<BlockExplodeEvent> legacyExplodeEventConstructor;
    private static Constructor<BlockExplodeEvent> modernExplodeEventConstructor;
    static {
        try {
            modernExplodeEventConstructor = BlockExplodeEvent.class.getConstructor(Block.class, BlockState.class, List.class, float.class, ExplosionResult.class);
        } catch (Exception ignored) {
            // Some runtimes still expose the pre-1.21 signature instead.
        }

        try {
            legacyExplodeEventConstructor = BlockExplodeEvent.class.getConstructor(Block.class, List.class, float.class);
        } catch (Exception ignored) {
            // Some runtimes only expose the newer signature.
        }

        if (modernExplodeEventConstructor == null && legacyExplodeEventConstructor == null) {
            Slimefun.logger().log(Level.SEVERE, "Could not find any compatible constructor for BlockExplodeEvent");
        }
    }
    ...
    private BlockExplodeEvent createNewBlockExplodeEvent(
        ...
        try {
            if (modernExplodeEventConstructor != null) {
                return modernExplodeEventConstructor.newInstance(block, block.getState(), blocks, yield, ExplosionResult.DESTROY);
            }

            if (legacyExplodeEventConstructor != null) {
                return legacyExplodeEventConstructor.newInstance(block, blocks, yield);
            }
        } catch (Exception e) {
            Slimefun.logger().log(Level.SEVERE, "Could not create BlockExplodeEvent", e);
        }
