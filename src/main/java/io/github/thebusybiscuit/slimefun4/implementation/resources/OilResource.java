...
class OilResource extends AbstractResource {
    ...
    OilResource() {
        super("oil", "Oil", getDisplayItem(), 8, false);

        MinecraftVersion version = Slimefun.getMinecraftVersion();

        if (version.isVirtual() || version.isAtLeast(MinecraftVersion.MINECRAFT_1_18)) {
    ...
    private static @Nonnull ItemStack getDisplayItem() {
        try {
            return SlimefunItems.OIL_BUCKET.item();
