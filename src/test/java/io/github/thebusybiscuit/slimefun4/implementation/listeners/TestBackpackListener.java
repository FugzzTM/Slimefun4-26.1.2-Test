...
class TestBackpackListener {
    ...
    private PlayerBackpack openMockBackpack(Player player, String id, int size) throws InterruptedException {
        ...
        try {
            Field backpacksField = BackpackListener.class.getDeclaredField("backpacks");
            backpacksField.setAccessible(true);

            @SuppressWarnings("unchecked")
            Map<UUID, ItemStack> backpacks = (Map<UUID, ItemStack>) backpacksField.get(listener);
            backpacks.put(player.getUniqueId(), item.item());
        } catch (ReflectiveOperationException ex) {
            throw new IllegalStateException("Could not mark backpack as open for test setup", ex);
        }
    ...
    private boolean isAllowed(String id, ItemStack item) throws InterruptedException {
        Player player = server.addPlayer();
        PlayerBackpack backpack = openMockBackpack(player, id, 9);
        Inventory inv = backpack.getInventory();

        int slot = 7;
        inv.setItem(slot, item);
        InventoryClickEvent event = new InventoryClickEvent(InventoryViewWrapper.wrap(player, id, inv, player.getInventory(), InventoryType.CHEST), SlotType.CONTAINER, slot, ClickType.LEFT, InventoryAction.PICKUP_ONE);
    ...
    void testHotbarKey(Material type) throws InterruptedException {
        Player player = server.addPlayer();
        PlayerBackpack backpack = openMockBackpack(player, "BACKPACK_HOTBAR_" + type.name(), 9);

        int slot = 7;
        player.getInventory().setItem(slot, new ItemStack(type));
        InventoryClickEvent event = new InventoryClickEvent(InventoryViewWrapper.wrap(player, "BACKPACK_HOTBAR_" + type.name(), backpack.getInventory(), player.getInventory(), InventoryType.CHEST), SlotType.CONTAINER, slot, ClickType.NUMBER_KEY, InventoryAction.PICKUP_ONE, slot);
        listener.onClick(event);
